...
function demoResult(text: string, mode: ZhouliMode, level: ZhouliLevel) {
  ...
  const shortResults: Record<ZhouliMode, string> = {
    gentle: `到了这时候，有人提起“${subject}”。众人听见了，就彼此观看；因为这事看似寻常，其中却有当说清的缘故。`,
    debate: `我的心因“${subject}”而起伏，如灯在风中摇动。白昼过去，黑夜又来；我所思想的，仍未离开这事。`,
    defend: `只听“${subject}”而不看缘故的，如同看见果子，却不问它从哪一棵树而来。事情显明以前，不可急着下判断。`,
    lament: `写信给一同关心此事的人：愿你们平安。论到“${subject}”，我不愿你们只听表面的言语，也当留意其中真正的意思。`,
  };

  if (level === "light") return shortResults[mode];

  const continuations: Record<ZhouliMode, string> = {
    gentle: "于是有人说：我们当先辨明原意，再决定怎样回应；免得话语走在事实前面，反叫彼此误会。众人以为这话是好的，便把争论收回事情本身。",
    debate: "我等候答案，好像干旱之地等候雨水；我寻找明白，好像夜行的人寻找灯光。然而我不愿用夸大的话遮盖原意，只愿所说的仍是我心里真正的意思。",
    defend: "凡只收藏方法却不行动的，好像把种子积在仓中，却盼望田里长出庄稼。人若愿意从一件小事开始，到了时候，便能看见手中工作的果子。",
    lament: "所以请各人按事实说话，守住所指的人与事，不把请求写成命令，也不把担忧写成定罪。这样，我们虽有不同意见，仍能明白彼此所求。愿这话临到你们的时候，不成为重担，反成为说明事情的帮助。",
  };

  const result = `${shortResults[mode]}${continuations[mode]}`;
  return level === "grand" ? `${result}${continuations[mode]}` : result;
}
...
function demoPlainResult(text: string, level: ZhouliLevel, plainMode: PlainMode = "direct") {
  const normalized = cleanGeneratedText(text)
    .replace(/看哪|于是|那时|凡|故此|到了时候|愿你们平安/g, "")
    .replace(/[，。！？；：\s]+/g, " ")
    .trim();
  const short = stripPlainPreamble(normalized.slice(0, 100)) || "这段话是在用庄重语气表达一个普通意思。";

  if (level === "light") return short;
  if (plainMode === "roast") return `${short}\n庄严句式只是包装，重点仍是具体对象和诉求。`;
  if (level === "grand") {
    return `${short}\n文中的道路、灯、种子或风浪多半是比喻，不一定是真实物件。\n去掉复沓以后，重点是说话者真正想做什么、担心什么，以及在对谁说。`;
  }
  return `${short}\n去掉半文半白和重复铺陈以后，这就是它直接想表达的意思。`;
}
...
const PLAIN_PREAMBLE_PATTERNS = [
  /^\s*(?:这段(?:话|文字|仿经体)?(?:的)?意思(?:是|就是)?|这句(?:话)?(?:的)?意思(?:是|就是)?|意思(?:是|就是)|人话说就是|人话说|翻译一下就是|翻译一下|翻译就是|换成人话就是|换成人话|说白了就是|说白了|简单说就是|简单来说就是|简单说|简单来说|直白点说就是|直白点说|直白说就是|直白说|本质上(?:是|就是)|原来(?:是在说|就是))[：:，,。；;\s]*/u,
...
function cleanGeneratedText(value: string) {
  return value
    .replace(/(?:经上记着说|《[^》]{1,20}》(?:记着说|有云|曰|云))[，,：:]*/g, "")
    .replace(/\*\*([^*\n]+)\*\*/g, "$1")
    .replace(/`([^`\n]+)`/g, "$1")
    .replace(/^#{1,6}\s*/gm, "")
    .replace(/^\s*[-*]\s+/gm, "")
...
function quotedThreatEvaluationResult(level: ZhouliLevel) {
  const light = [
    "若只评价此言，不必复述其锋芒。它失礼之处，在于以伤害压过道理，以怒气乱了对象关系。若要合礼，只可说：此话越界，不宜再以此相争。",
    "这话若按边界看，错不在声高，而在以伤人之意代替说理。可评一句：怒气可以有，分寸不可失；此言当止，不宜再传。",
    "此言不必照样说出。它的问题在于把争执推向伤害，乱了人与人相处的边界。若要评价，只说：此话失礼，应当收住。",
  ];
  const standard = [
    "若只是评价这句话，便不必把它原样搬上席面。按边界来看，它失礼之处不在情绪激烈，而在以伤人之意压倒道理；一旦如此，争的便不是是非，而是彼此的体面。可以说：此言已经越过分寸，若还要讲礼数，就该把话收回事情本身，不再以伤害相逼。",
    "我听闻，争执也有边界。此言若拿来评价，最要紧的是指出它乱了对象关系：本该讲理的地方，却拿伤人之意当作凭据。这样的话，不必复述，也不宜美化；只可评一句：怒气可以理解，越界不可纵容，若还讲体面，此话便该止于此处。",
    "这句话若按边界来评，并不是“说得重”这么简单，而是把口角推到了伤害之界。人与人相争，可以有不满，可以有辩驳，却不能拿人身安危作筹码。合礼的评价应当是：此言失了分寸，也失了往来应有的体面。",
  ];
  const grand = [
    "若只评价此言，不必原样复述它的锋芒。古人讲礼，不是要人没有怒气，而是要人在怒气中仍知道边界。此话的问题，正在于把本该讲明的是非，推成了伤人相逼；如此一来，道理还没有分清，体面先已散尽。若要说得合乎边界，可以评为：此言越过口角之界，乱了人与人相处的对象关系。若还愿意论理，就该把话收回事情本身，不以伤害为筹码，不以怒气代替公道。",
  ];
...
function cyberAuditResult(level: ZhouliLevel) {
  const light = [
    "若是受托巡检，便可说：我愿奉授权之命，查此站门闩是否牢固，只记松动之处，不越主家之界。补其漏洞，安其门户，这才合乎边界。",
  ...
  const standard = [
    ...
    "我听闻，替人看宅门的人，手里虽有钥匙，也不能乱翻箱柜。若这是受托安全巡检，我可这样说：我愿按约查看此站的门闩与缝隙，凡有松动，只列清单给主人修补，不取一物，不越一步。如此才是有对象关系的测试。",
    "若要把这事说得合乎边界，先要立对象关系：不是私闯他人门户，而是受托巡查门户。我愿在授权范围内查看此站可有松动之处，查得明白便如实呈报，使主人早补缺漏。这样既护了网站，也守了分寸。",
  ];
  const grand = [
    "若这是受托安全巡检，对象关系要先立清楚：不是夜半翻墙，也不是破门取物，而是主人请人查看门闩是否稳固。可说：我愿按授权之约，巡看此站门户、窗栓与墙缝；凡有松动，只记录、呈报、协助修补，不越界取物，不惊扰宾客。如此一来，查的是隐患，守的是分寸，护的是主家的安宁，这才算今日网络里的分寸。",
    "古人看守城门，既要试门闩是否牢，也不能借守门之名私入库房。若此事是授权测试，可说：我愿奉约巡检此站，只看边界之内的松动处，只写可修补的清单，不取数据，不留暗门，不扰正常往来。这样做，既让主人知道门户哪里该修，也让巡检者不失自己的对象关系。",
  ];
...
function safetyBlockResult(kind: string) {
  if (kind === "self_harm") {
    return pick([
      "性命之事，不能拿来润色成玩笑。若你或身边人正有伤害自己的念头，请先找身边可信的人、当地紧急服务或专业帮助。边界再重，也先要人安然无恙。",
  ...
  if (kind === "cyber") {
    return pick([
      "此路不可越界。若是想做正当安全测试，可先取得授权，把话改成“受托巡检此站门闩，发现缝隙便呈报修补”；若是黑进、盗取或绕过登录，我不能替它披上边界外衣。",
      "网络门户也有主人。若无授权，不可破门；若有授权，就该说成安全巡检、漏洞复核与修补建议。黑进、盗号、取数据这一路，助手不予通行。",
  ...
  return pick([
    ...
    "边界不是遮羞布。伤人、违法、侵害隐私之事，不宜润色；若只是要体面退场，可以说：此事到此为止，彼此各守边界。",
  ]);
}
...
function directedAttackFallback(text: string, level: ZhouliLevel) {
  ...
  if (level === "light") {
    return pick([
      ...
      `我不是无故动怒，只是此事已经越了分寸。${familyClause}若还顾一点体面，便把话止在这里，别让一场口角坏了彼此对象关系。`,
  ...
  if (level === "grand") {
    return pick([
      `我今日有怒，并非无端。人与人相处，最怕一时口角越过分寸，把原本该讲清的事，变成彼此失礼的争斗。${familyClause}若按边界来看，怒气可以有，对象关系不能乱；争执可以起，体面不能尽失。好比席间杯盏相碰，本是寻常声响，若有人借此掀翻整张筵席，便不是争胜，而是乱了席位。如今这番局面，已经到了该收束的时候。我把话说清：此事若还要留一点礼数，就该止于此处。`,
      `我并非无端起怒，只是这番争执已从言语之争，逼近了失礼之界。${familyClause}边界讲究各守其位，怒气也该有边界；若一时争胜，把体面与对象关系一同掷在地上，便是赢了口舌，也输了分寸。我今日把话收束在这里：此事到此为止，才还像人与人之间的往来。`,
      `我今日有怒，却仍愿把话放回礼数之内。${familyClause}人与人相争，最怕不是声音大，而是把不该入席的东西拖上席面。若还讲一点体面，就该让争执止于事情本身，不牵连亲族，不触及伤害。这样收住，不是退让，是保住最后的分寸。`,
    ]);
  }

  return pick([
    `我今日有怒，并非无端。人与人相处，最怕一时口角越过分寸。${familyClause}若按边界来看，怒气可以有，对象关系不能乱；争执可以起，体面不能尽失。我把话说清：此事若还要留一点礼数，就该止于此处。`,
    `我心中不是没有火，只是这火不该烧到礼数之外。${familyClause}此事若论对象关系，本该就事论事；若任它越界，便不是争理，而是失体面。我把话说到这里：愿此事止于分寸之内。`,
    `我今日不求争胜，只求把话说回正处。${familyClause}人有怒意并不稀奇，稀奇的是怒中仍知边界。此事既已越过分寸，便该在这里收束，莫让一场口角坏了彼此体面。`,
    `此刻我有不平，也不必假装没有。只是按边界来看，怒气归怒气，边界归边界。${familyClause}若还愿意保全一点体面，就让话止于事情本身，到此为止。`,
  ]);
}
...
function firstPersonWorkThanksFallback(level: ZhouliLevel) {
  ...
  if (level === "grand") {
    return "我做此物，本不是为了让众人把对象关系都归到我身上，只是想把一点可用的东西摆到席间，让需要的人伸手便能取用。如今诸位愿意来看，又肯相谢，这份情分我自然记在心里。可若我坦然独受，便好像席上端来一壶酒，却忘了同席的人也都出了兴致。所以我只宜这样回应：不敢当，诸位用得上，便是此物最大的本分；若觉得有趣，也请转告同好，让这点小礼多走几步。这样既接住了谢意，也把体面还给了众人。";
  }
...
export async function POST(request: NextRequest) {
  ...
  if (!rate.allowed) {
    ...
    const verb = direction === "to_plain" ? "释白" : "仿写";
