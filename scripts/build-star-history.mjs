...
const repo = process.env.STAR_HISTORY_REPO || "asdisadsg/speak-biblical";
const outputPath =
  process.env.STAR_HISTORY_OUTPUT ||
  path.join("public", "images", "github-star-history-biblical.svg");
const historyPath =
  process.env.STAR_HISTORY_DATA ||
  path.join("public", "images", "github-star-history-biblical.json");
...
async function requestJson(url, accept = "application/vnd.github+json") {
  const headers = {
    Accept: accept,
    "User-Agent": "speak-biblical-star-history",
  };
...
function buildSvg({ repoInfo, stargazers, generatedAt }) {
  ...
  <text x="${width / 2}" y="${height / 2 + 44}" text-anchor="middle" fill="#8c342a" fill-opacity="0.06" font-family="Songti SC, STSong, SimSun, serif" font-size="280" font-weight="700">卷</text>
  <text x="${headerLeft}" y="58" fill="#211d18" font-family="Songti SC, STSong, SimSun, serif" font-size="34" font-weight="700">星数记</text>
